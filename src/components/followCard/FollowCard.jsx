...
const FollowCard = ({ follow, followingList }) => {
  ...
  const followingStatus = FollowingListOfLoggedInUser?.findIndex(
    (followingUser) => followingUser.userId == followId
  );
  ...
  return (
    <div
      className="w-full lg:w-[85%] bg-bg-color border border-solid border-slate-500 rounded-3xl p-5 flex gap-4 justify-between items-center cursor-pointer"
      onClick={fetchUserPosts}
    >
      <div className="flex justify-center gap-4">
        <div className="">
          {follow?.profileIconImg ? (
            <img
              className="inline-block h-[60px] w-[60px] rounded-full object-cover "
              src={follow?.profileIconImg}
            />
          ) : (
            <img className="cameraIcon1" src={cameraIcon} alt="" />
          )}
        </div>
